...
a7<-a6 %>%
  group_by(treatment,taxa,sampling,season,year)%>%
...
ggplot(data=a7)+
  ...
  scale_x_continuous(name="Year",breaks=c(1,2),label = c(1,2))+
  # geom_ribbon(data = p1, aes(year,
  #   ymin = conf.low,
  #   ymax = conf.high, 
  #   group = season),
  #   alpha = 0.3
  #   ) + 
  # geom_line(data = p1, aes(year, predicted, group = season)) +
  geom_errorbar(data=a7,
                aes(x=year,
                    ymin=m.abund-se.abund,
                    ymax=m.abund+se.abund,
                    color=treatment),
                position=position_dodge(0.5),
                width=.3)+
  geom_point(aes(x=year,y=m.abund,color=treatment), size=3,alpha =0.8,
    position=position_dodge(0.5))+

  facet_grid(taxa~season,scales = "free")+
  theme_bw()+
  ...
  ylab(ylab)+
  scale_color_brewer(palette = "Dark2",name="Treatment",labels=c("Control",
    "Structure Contol",
    "Sponge"))
