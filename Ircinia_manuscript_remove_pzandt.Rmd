...
In other ecosystems, sponges contribute to the maintenance of diverse and productive systems through their facilitation of foundation species (e.g., mangroves) and the retention and recycling Sponges are common in tropical and subtropical seagrass beds, yet we know little about how their presence impacts these communities.
...
Sponge presence increased seagrass nutrient content and growth, as well as increased abundance of macroalgae and non-dominant seagrass species (*Syringodium filiforme* and *Halodule wrightii*). 
...
However, foundation species are often facilitated by members of their associated assemblages [@gagnon2020; @peterson2013; @ellison1996].
...
Seagrass beds are important coastal ecosystem worldwide. 
...
*Ircinia felix* is a high microbial abundance sponge, indicating it hosts a dense and diverse microbiome [@weisz2008]. 
...
A single sponge (*I. felix*, average volume ± standard deviation, 2.5 ± 0.75 L) was placed inside a cage in the center of each sponge plot.
...
Growth rates of *T. testudinum* shoots were measured at four distances from the center of each plot: the center of the plot or immediately next to the sponge/sponge model (designated as 0 m) and in permanently marked points (using stakes) at 0.5, 1.0, and 2.0 m from the center of the plot. 
...
Seagrass shoots did not fluctuate between seasons, so we included fixed effects of months into the experiment in a three-way interaction with treatment and distance (a linear covariate representing the center of contiguous 1 m^2^ sampling quadrats). 
Because species-specific shoot counts were not collected for *S. filliforme* or *H. wrightii*, shoot density was modeled for *T. testudinum* alone as the dominant species and *S. filliforme* and *H. wrightii* combined.  
...
Because an effect was detected at 0 and 0.5 m and not at the 1 m or 2 m sampling points, the distance was included in this model as a factor with 0 and 0.5 m assigned as "near" and 1 and 2 m assigned as "far."\par  
...
Macroalgal abundances did not change significantly between the first and second years of the experiment in control or structure plots (Fig. \@ref(fig:algaep)); macroalgal abundances increased in the sponge treatment in year two of the experiment ($\beta$ = `r efsize.tmb(alm1r.sum,4)`, Fig. \@ref(fig:algaep); Fig. S2). 
The control and structure plots both increased significantly less than the sponge treatment ($\beta$ = `r efsize.tmb(alm1r.sum,6)` and $\beta$ = `r efsize.tmb(alm1r.sum,7)` respectively) and did not differ from each other (Table S1).
...
Seagrass growth was slower in the winter ($\beta$ = `r efsize(sgr,6)`) and the decrease in growth during the winter was not different among treatments. 
...
Before the experiment, seagrass in sponge plots had significantly lower nitrogen concentrations than control plots (Fig. \@ref(fig:nuts)A, $\beta$ = `r efsize(nbs,3)`) but not structure plots. 
...
In sponge plots, % carbon showed a slight increase in seagrass tissues, resulting in a significantly different response than structure plots ($\beta$ = `r efsize(crls,6)`) but not control plots.  
...
However, the pattern of change over time was significantly different between control and sponge plots ($\beta$ = `r efsize(pbs,6)`), but not between structure and sponge plots or control and structure plots. 
