...
Recent theoretical work **has highlighted the potential for- sounds positive when I think the destabilization is negative. I would use "has emphasized the potenial for" or "has suggested** suggests that facilitative interactions involving foundation species have potential to be destabilizing in the face of anthropogenic change.
Consequently, it is becoming more important to understand which taxa facilitate seagrasses.
In other ecosystems, sponges contribute to the maintenance of more diverse and productive systems through their facilitation of foundation species (e.g., mangroves) and the retention and recycling of energy and nutrients within the system. 
While sponges are also common in seagrass beds, we know little about how their presence impacts these communities. **is this true for all seagrass communities, not just T. testudinum ones?**
...
Sponge presence resulted in an increase in seagrass nutrient content and growth, and increased abundance of macroalgae and non-dominant seagrass species (*Syringodium filiforme* and *Halodule wrightii*). 
These changes were not seen in our control (unmanipulated) or structure control (where we placed a polypropylene sponge replica) plots. 
...
Our study shows that sponges can have a positive influence on seagrass bed foundation species in unimpacted systems.
Further work is needed to understand how this facilitation impacts the stability of seagrass beds in areas where human activities have increased ambient nutrient levels. **Is it worth highlighting that there is evidence that sponges act differently at high nutrient levels, and that this contrasts with patterns in other community members? Without some hint to that effect, the second sentence feels a bit unaddressed...**
...
Nutrient content (%C, %N, and %P) was assessed for 10 shoots growing within 0.25 m of the center of each plot, prior to the beginning of the experiment and again after 1 year. 
...
*Thalassia testudinum* shoot densities decreased similarly in all treatments over time, but this decrease was not significant in sponge plots (control: $\beta$ = `r efsize.tmb(tsdb.sum,4)`; structure control: $\beta$ = `r efsize.tmb(tsdf.sum,4)`; sponge: $\beta$ = `r efsize.tmb(tsdr.sum,4)`). 
...
Conversely, in sponge plots seagrass % N responded differently than both the control ($\beta$ = `r efsize(nrs,5)`) and structure control ($\beta$ = `r efsize(nrs,6)`) plots. Sponge plots had a higher % N after one year (Fig. \@ref(fig:nuts)A), however, this increase was not significant.  
  
The pattern was similar for percent carbon; in sponge plots seagrass % C  was initially lower than control (Fig. \@ref(fig:nuts)B, $\beta$ = `r efsize(cbs,3)`) and structure control plots ($\beta$ = `r efsize(cfs,3)`). 
...
Specifically, the presence of a sponge resulted in increased seagrass nutrient content and growth, and in an increased abundance of both macroalgae and seagrasses.\par  
...
Because primary producers are typically limited by both nitrogen and phosphorus in our study system [@allgeier2010], we hypothesized that the sponges we transplanted would supply these nutrients resulting in the facilitation of seagrass bed primary producers. 
Consistent with our hypothesis, we saw an increase in seagrass nutrient content in our plots with live sponges.
Ours is not the first study to find that sponge-released nutrients can facilitate primary producers.
For example, sponges growing on mangrove roots supply nutrients to the mangrove [@ellison1996] and sponges can supply nitrogen to macroalgae on coral reefs [@easson2014]. 
Further, @archer2015 showed that a different sponge (*Halichondria melanadocia*) that grows around the base and leaves of seagrass shoots likely provide nutrients to those shoots. 
However, this study, to the best of our knowledge, is the first to show that a single massive form sponge can increase nutrient content in seagrasses within a 0.25 m radius. \par

The other changes we documented in the primary producers in our sponge plots are consistent with an increase in nutrient supply in an oligotrophic system. 
...
However, similar structures (a sponge replica and holding cage) were added to our structure control plots and the response of the primary producers in control and structure control plots did not differ significantly while we saw a significant response in our sponge plots for most responses. 
This suggests that it is a function of the live sponges, rather than the presence of structure, that lead to increased seagrass nutrient content and growth, and increased abundance of both seagrasses (*S. filliforme* and *H. wrightii*) and macroalgae.\par

While our study is the first to show facilitation of primary producers in seagrass beds by sponges; seagrasses are often facilitated by other filter feeders increasing nutrient availability [@gagnon2020].
For example, Reusch and coauthors [-@reusch1994] found that the blue mussel (*Mytilus edulis*) facilitates seagrass growth via fertilization of sediments through the deposition of biodeposits (feces and pseudofeces).
However, this effect appears to be context-dependent.
Specifically, in eutrophic conditions the presence of mussels leads to reduced seagrass density due to high sulfide concentrations in the sediments resulting from the combined effects of water column nutrient enrichment and mussel biodeposits.
The effect of the epiphytic sponge *H. melanadocia* was also found to be partially determined by the sponge's supply of limiting nutrients [@archer 2015; @archer2018c].
Under oligotrophic conditions this sponge-seagrass interaction is commensal, with the seagrass providing an attachment point for the sponge and the seagrass displaying no net effect of sponge presence.
The net neutral effect of the sponge on the seagrass is, however, a balance of a negative effect of the sponge shading the seagrass with the sponge releasing bioavailable forms of limiting nutrients (N and P) [@archer2015].
This interaction is also context dependent, with small increases in ambient nutrient levels resulting in a shift from commensalism to parasitism and a reduction of seagrass growth and biomass [@archer2018c].\par
...
For example, these interactions can lower the threshold of nutrient pollution that leads to decline in seagrass ecosystems, driving seagrass loss at lower nutrient levels than would be predicted by studying seagrasses in isolation.
On first glance, it may appear that the facilitation of primary producers by *I. felix* would be no different, as this facilitation appears to be based on nutrients supplied by the sponge. 
However, nutrient processing by the *I. felix* holobiont is also context-dependent. 
While there is little spatial or temporal variability in *I. felix*'s symbiotic microbiome [@erwin2012]; the active portion of the microbiome appears to be dependent on ambient nutrient concentrations [@archer2017].
...
This context-dependent nutrient processing has been documented in other sponges as well [@pawlik2020].
While the ecosystem-level effects of this context-dependent nutrient processing have not been studied, it is reasonable to predict that it should have a stabilizing effect on ambient nutrient levels when sponges are present in sufficient densities.
This study showed facilitation of primary production in a seagrass bed by *I. felix* under oligotrophic conditions, likely due to nutrients supplied by the sponge. 
...
Overall, our study shows that sponges can facilitate seagrass bed primary producers and that this facilitation is likely based on nutrients supplied by the sponge.
It is important to understand which taxa facilitate foundation spcies, like seagrass, because this can have cascading consequences on local diversity, ecosystem function, and the delivery of ecosystem services.
Theoretical and empirical work shows that interactions involving nutrient-transfer are often context-dependent and that such interactions involving foundation species can lead to non-linear ecosystem dynamics when human activities significantly alter ambient nutrient levels. 
We studied the effect of *I. felix* in unimpacted seagrass beds.
Therefore, this study represents a first step in understanding how sponges influence seagrass ecosystems and our results suggest that further work is necessary to determine if this facilitation breaks down in impacted systems.
...
```{r shootd, out.width="55%", echo=FALSE,message=FALSE, fig.cap="Shoot densities of (a) *T. testudinum* and (b) *S. filliforme* and *H. wrightii* combined over the course of the experiment."}
knitr::include_graphics("figures/shootdensity-95CI.jpg")
```


```{r grow, out.width="85%", echo=FALSE,message=FALSE, fig.cap="Segrass growth in the summer (a) and winter (b) over the course of the experiment. Summer sampling happened in months 1 and 12 (years 1 and 2 respectively) while winter sampling occurred in months 5 and 17."}
knitr::include_graphics("figures/sggrow-95CI.jpg")
```


```{r nuts, out.width="65%", echo=FALSE,message=FALSE, fig.cap="Seagrass nutrient concentrations prior to the initiation of the experiment and at one year (12 months) into the experiment. (a) Percent nitrogen, (b) percent carbon, and (c) percent phosphorus in *Thalassia testudinum* tissue."}
