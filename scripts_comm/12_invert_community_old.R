...
# Organize data
i.env <- i2 %>%
  select(treatment, plot, yr, sampling, season) %>%
  left_join(alg) %>%
  left_join(sg) %>%
  left_join((sggrow))
...
trt0.mat <- data.frame(model.matrix(~trt0,
  contrasts = list(trt0 = "contr.helmert")
))[, -1]
...
tr.samp.mat <- data.frame(model.matrix(~ samp * trt + plts,
  contrasts = list(trt = "contr.helmert")
))[, -1]
...
tr.samp.mat.np <- data.frame(model.matrix(~ samp * trt,
  contrasts = list(trt = "contr.helmert", seas = "contr.helmert")
))[, -1]
...
tr.s.yr.mat <- data.frame(model.matrix(~ yr * seas * trt + plts,
  contrasts = list(trt = "contr.helmert", seas = "contr.helmert", yr = "contr.helmert")
))[, -1]
...
tr.yr.mat <- data.frame(model.matrix(~ yr * trt, # without plot
  contrasts = list(trt = "contr.helmert", yr = "contr.helmert")
))[, -1]
...
yr.seas.mat <- data.frame(model.matrix(~ yr * seas, # without plot
  contrasts = list(yr = "contr.helmert", seas = "contr.helmert")
))[, -1]
...
sg.s.yr.mat.nop <- data.frame(model.matrix(~ yr * seas * sggrow,
  contrasts = list(seas = "contr.helmert", yr = "contr.helmert")
))[, -1]
...
icom17diff <- bind_cols(
  i.com17[, colnames(i.com17) %in% c(
    "blue crab",
    "Anemone",
    "cerith",
    "sea cucumber",
...
ggplot(data = icom17diff) +
  geom_col(aes(x = taxa, y = n.plots, fill = treatment),
    width = .5,
...
(i0 <- ggplot() +
  ...
  geom_segment(
    data = sprp0, aes(x = 0, xend = PC1, y = 0, yend = PC2),
  ...
  #     ),
  geom_text_repel(
    min.segment.length = 5, # point.padding = 0.2, direction = "both",
  ...
  #              level = 0.95)+
  stat_ellipse(
    geom = "polygon", aes(x = PC1, y = PC2, fill = treatment),
...
(i0hull <- ggplot() +
  ...
  geom_jitter(
    data = i.env0p, aes(x = PC1, y = PC2, color = treatment),
...
(i5 <- ggplot() +
  ...
  geom_path(data = circ, aes(x, y), lty = 2, color = "grey", alpha = 0.7) +
  geom_segment(
    data = sprp5, aes(x = 0, xend = PC1, y = 0, yend = PC2),
    arrow = arrow(length = unit(0.025, "npc"), type = "open"),
    lwd = .5
  ) +
  # geom_text(check_overlap = T,
  ...
  #    geom_polygon(data = hull0, aes(x=PC1,y=PC2,color=treatment,fill=treatment),alpha = 0.1)+
  geom_jitter(
    data = i.env5p, aes(x = PC1, y = PC2, color = treatment),
...
(i12 <- ggplot() +
  ...
  geom_path(data = circ, aes(x, y), lty = 2, color = "grey", alpha = 0.7) +
  geom_segment(
    data = sprp12, aes(x = 0, xend = PC1, y = 0, yend = PC2),
    arrow = arrow(length = unit(0.025, "npc"), type = "open"),
    lwd = .5
  ) +
  # geom_text(check_overlap = T,
...
(i12hull <- ggplot() +
  ...
  geom_jitter(
    data = i.env12p, aes(x = PC1, y = PC2, color = treatment),
    width = 0.01, height = 0.01, alpha = 0.75, size = 2
  ) +
  geom_segment(
    data = sprp12, aes(x = 0, xend = PC1, y = 0, yend = PC2),
...
(i17 <- ggplot() +
  ...
  #   alpha = 0.1)+
  geom_jitter(
    data = i.env17p, aes(x = PC1, y = PC2, color = treatment),
    width = 0.01, height = 0.01, alpha = 0.75, size = 2
  ) +
  geom_segment(
    data = sprp17, aes(x = 0, xend = PC1, y = 0, yend = PC2),
    arrow = arrow(length = unit(0.025, "npc"), type = "open"),
    lwd = .5
  ) +
  geom_text_repel(
