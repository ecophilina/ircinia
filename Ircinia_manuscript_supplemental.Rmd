---
title: 'Individual sponges alter community composition via facilitation in a Bahamas seagrass system: Supplemental Material'
...
return(knitr::kable(tab1,caption=cap))
}
```

```{r alg.table,echo=FALSE,warning=FALSE,message=FALSE}
tab.func(type="glmmtmb",msum=alm1r.sum,cap="Macroalgal abundance model results.")
```
  
  
```{r tsd.table,echo=FALSE,warning=FALSE,message=FALSE}
tab.func(type="glmmtmb",msum=tsdr.sum,cap="T. testudinum shoot density model results.")
```  

```{r shsd.table,echo=FALSE,warning=FALSE,message=FALSE}
tab.func(type="glmmtmb",msum=shsdr.sum,cap="S. filliforme and H. wrightii shoot density model results.")
```  

```{r grow.table,echo=FALSE,warning=FALSE,message=FALSE}
tab.func(type="lmer",msum=sgr,cap="Seagrass growth model results.")
```  

```{r pn.table,echo=FALSE,warning=FALSE,message=FALSE}
tab.func(type="lmer",msum=nrs,cap="Percent nitrogen in seagrass tissue model results.")
```  

```{r pc.table,echo=FALSE,warning=FALSE,message=FALSE}
tab.func(type="lmer",msum=crls,cap="Percent carbon in seagrass tissue model results.")
```  

```{r pp.table,echo=FALSE,warning=FALSE,message=FALSE}
tab.func(type="lmer",msum=prs,cap="Percent phosphorus in seagrass tissue model results.")
```