...
Macroalgal abundances decreased in the winter in all treatments, however, the decrease was only significant in control ($\beta$ = `r efsize.tmb(alm1b.sum,5)`, z `r tmb.res(alm1b.sum,5)`). 
Macroalgal abundances did not change significantly between the first and second year of the experiment in control or structure control plots, \@ref(fig:algaep)). **note if we keep this figure, we should add panel labels to it** 
...
*Thalassia testudinum* shoot densities decreased similarly in all treatments over time, but this decrease was not significant in sponge plots (control: $\beta$ = `r efsize.tmb(tsdb.sum,4)`, z `r tmb.res(tsdb.sum,4)`; structure control: $\beta$ = `r efsize.tmb(tsdf.sum,4)`, z `r tmb.res(tsdf.sum,4)`). 
There was no significant effect of distance for any treatment initially or over time. 
*Syringodium filiforme* and *H. wrightii* shoot densities did not change in control or structure control plots, but increased in sponge plots ( $\beta$ = `r efsize.tmb(shsdr.sum,4)`, z `r tmb.res(shsdr.sum,4)`). 
...
Seagrass growth was slower in the winter ($\beta$ = `r efsize(sgr,6)`, t~`r aov.df(sgr$coefficients,6)`~ `r aov.res(sgr$coefficients,6)`) and the decrease in growth during the winter was not different between treatments. 
...
In contrast, seagrass growth declined in the control and structure control plots , but this decline was only significant in the control ($\beta$ = `r efsize(sgb,5)`, t~`r aov.df(sgb$coefficients,5)`~ `r aov.res(sgb$coefficients,5)`, Fig. \@ref(fig:grow)).  
...
Before the the experiment, seagrass in sponge plots had significantly lower nitrogen concentrations than control plots (Fig. \@ref(fig:nuts)A, $\beta$ = `r efsize(nbs,3)`, t~`r aov.df(nbs$coefficients,3)`~ `r aov.res(nbs$coefficients,3)`) but not structure control plots. 
Percent nitrogen in seagrass tissues in both control and structure control plots declined similarly over time (Fig. \@ref(fig:nuts)A), but the decrease was only significant in control plots (control: $\beta$ = `r efsize(nbs,4)`, t~`r aov.df(nbs$coefficients,4)`~ `r aov.res(nbs$coefficients,4)`). 
Conversely, seagrass in sponge plots responded differently than both the control ($\beta$ = `r efsize(nrs,5)`, t~`r aov.df(nrs$coefficients,5)`~ `r aov.res(nrs$coefficients,5)`) and structure control ($\beta$ = `r efsize(nrs,6)`, t~`r aov.df(nrs$coefficients,6)`~ `r aov.res(nrs$coefficients,6)`) plots. Sponge plots had a higher % nitrogen after one year (Fig. \@ref(fig:nuts)A), however, this increase was not significant.  
...
However, this decrease was only significant in structure control plots ($\beta$ = `r efsize(cfs,4)`, t~`r aov.df(cfs$coefficients,4)`~ `r aov.res(cfs$coefficients,4)`). 
In sponge plots, % carbon showed a slight increase in seagrass tissues, resulting in a significantly different response than structure control plots ($\beta$ = `r efsize(crls,6)`, t~`r aov.df(crls$coefficients,6)`~ `r aov.res(crls$coefficients,6)`) but not control plots.  
...
However, the pattern of change over time was significantly different between control and sponge plots ($\beta$ = `r efsize(pbs,6)`, t~`r aov.df(pbs$coefficients,6)`~ `r aov.res(pbs$coefficients,6)`), but not between structure control and sponge plots or control and structure control plots. 
