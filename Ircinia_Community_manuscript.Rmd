...
Sponge plots while also retained more fish taxa (F~`r aov.df(fishloss12[[1]], 1)`~ `r aov.res(fishloss12[[1]], 1)` with sponge-control = `r tukey.res(fishloss12.tuk[[1]], 2)` and sponge-structure = `r tukey.res(fishloss12.tuk[[1]], 3)` while structure-control = `r tukey.res(fishloss12.tuk[[1]], 1)`); Fig. \@ref(fig:turnover-plots) B right panel).
...
While all sponge plots shifted a similar amount in a similar direction, control and structure control plots shifted in inconsistent directions and amounts (Fig. \@ref(fig:vector-plots).
\par
...
Although the sponge presence model explained the most variance for invertebrate taxa richness and abundance, other models fell within less than 2 $\delta \Delta$ AICc values that cannot be ruled out: macroalgal structure, sponge presence + seagrass productivity, and sponge presence + seagrass structure(Table 1). 
...
Even more so, their role in providing structural complexity, increasing habitat availability, and enhancement of primary producers help alleviate taxa loss in sub-tropical seagrass beds [@Ferrari2018;@Gratwicke2005;@Wulff2006; @Taylor2007; @Bell2008; @Archer2021]. \par
