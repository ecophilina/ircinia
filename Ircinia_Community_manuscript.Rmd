...
First, for each type of community data, a Hellinger's transformation was performed followed by an RDA using the ___ package(CITATION).
The scores from the RDA analysis were then used to calculate a vector length and angle for each plot as per Mccune. 
An analysis of variance was performed on each community's vector length and angle to test significant difference in treatments with real as the reference using the ____ package.
When significance occurred, a post-hoc analysis via a Tukey test was then performed to look at the differences in vector length and angle among treatments(CITATION).
...
<!-- In order to assess community changes among the three treatment groups, we created compositional vectors, where each treatment plot represented a single vector.  -->
<!-- Fish, non-clonal invertebrate, and clonal invertebrate community data were differences in compositional vector's angle (direction) and length across species space.  -->
<!-- To analyze vector length (differences in rate of change across species space) we used an linear model to test for differences among treatments, with the sponge treatment plots as the reference level through the stats package[@RCoreTeam2021].  -->
...
```  
<!-- At the end of the 12 months, the change in the fish community from the beginning of the experiment (i.e. vector length) was largest in the sponge treatment plots ($\beta$ = `r efsize(fish.vl.aov.sum,1)`). -->
<!-- This was significantly greater than the change in the fish communities within the control and structure treatment plots ( $\beta$ = `r efsize(fish.vl.aov.sum,2)`; $\beta$ = `r efsize(fish.vl.aov.sum,3)` respectively, Fig. \@ref(fig:vector-plots)). -->
<!-- The direction that the fish community shifted from the start of the experiment (i.e. vector angle) in the structure treatment plots was significantly different from the sponge treatment plots ($\beta$ = `r efsize(fish.angle.aov.sum,3)`, Fig. \@ref(fig:vector-plots)). -->
<!-- However, the change in fish community direction in the control plots was not significant, and may be due the the small rate of change in the community (Fig. \@ref(fig:vector-plots)).  -->
